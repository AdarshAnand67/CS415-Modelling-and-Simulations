...
class Robot:
    ...
    def __init__(self, env, name, battery_level):
        ...
        self.depletion_rate = random.uniform(1,5) # battery depletion rate of the robot
    ...
    def run(self):
        ...
        while True:
            if self.state == "Idle":
                ...
                time_work_done = (self.battery_level-10)/self.depletion_rate # time taken to work
                
                what_happened = yield (env.timeout(self.work_remaining) | env.timeout(time_work_done)) # wait for the job to be done or for the battery to be depleted
...
robot_list = [Robot(env, f"Robot #{i}", 100) for i in range(NUMBER_OF_ROBOTS)] # list of robots in the warehouse 
Job_Queue = simpy.Container(env, capacity=1000) # job queue (1000 jobs can be queued up)
