    ...
    if not server.is_available_spaces():
        ...
        with server.resource.request() as request:
            ...
            if request in results:
                ...
                print(
                    "[{:6.2f}:{}] - begin buying ticket after waiting for {:4.2f} time units".format(
                ...
                print(
                    "[{:6.2f}:{}] - wait for too long, leave the queue".format(
                        env.now, name
                    )
                )
...
ticket_office = Server(
    env, "office", capacity=1, service_rate=service_rate, queue_limit=QUEUE_LIMIT
)
