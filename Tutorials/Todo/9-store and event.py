...
class Server(object):
    ...
    def print_stats(self):
        print(
            "\t[{}] {} using, {} in queue".format(
                self.name, self.resource.count, len(self.resource.queue)
            )
        )
...
def Passenger(env, name, ticket_machine, ticket_office, gate):
    ...
    if random.random() < 0.8:
        ...
        with ticket_machine.resource.request() as request:
            ...
            print(
                "[{:6.2f}:{}] - reach machine after waiting for {:4.2f} time units".format(
                    env.now, name, t_queue
                )
            )
            service_time = ticket_machine.get_service_time()
            yield env.timeout(service_time)
            print(
                "[{:6.2f}:{}] - finish buying ticket after {:4.2f} time units".format(
        ...
        with ticket_office.resource.request() as request:
            ...
            t_queue = env.now - t_arrival
            print(
                "[{:6.2f}:{}] - reach counter after waiting for {:4.2f} time units".format(
                    env.now, name, t_queue
                )
            )
    ...
    with gate.resource.request() as request:
        ...
        t_queue = env.now - t_arrival
        print(
            "[{:6.2f}:{}] - reach the gate after waiting for {:4.2f} time units".format(
                env.now, name, t_queue
            )
        )
        ...
        yield env.timeout(service_time)
        print(
            "[{:6.2f}:{}] - pass the gate after {:4.2f} time units".format(
                env.now, name, service_time
            )
        )
...
def train(env, on_board, remaining, platform):
    ...
    print(
        "[{:6.2f}:Train] - train has arrived with {} pax on board, {} seats remaining, and {} pax waiting on the platform".format(
            env.now, on_board, remaining, n_pax_wait
        )
    )
    ...
    print(
        "[{:6.2f}:Train] - train leaves the platform with {} pax ({} still waiting on platform)".format(
            env.now, on_board, len(platform.items)
        )
    )
